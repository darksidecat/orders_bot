...
async def user_start_unregistered_user(m: Message):
    await m.answer(f"You are not registered. Please, contact bot administrator. Your id is {fmt.pre(m.from_user.id)}")
...
def register_start(dp: Dispatcher):
    dp.message.register(user_start_unregistered_user, CommandStart(),  any_state, AccessLevelFilter(access_levels=[]))
    dp.message.register(user_start_unregistered_user, any_state, AccessLevelFilter(access_levels=[LevelName.BLOCKED]))
    dp.callback_query.register(user_unregister_or_blocked, AccessLevelFilter(access_levels=[LevelName.BLOCKED]))
    dp.message.register(user_start, CommandStart(), any_state, AccessLevelFilter(access_levels=list(LevelName)))
